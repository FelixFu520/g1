            ...
            try:
                ...
                async for audio_chunk in audio_stream:
                    ...
                    if wait_for_chunk > 3.0:
                        logger.error(f"sender等待音频数据耗时过长: {wait_for_chunk:.3f}s, "
                                    f"极可能导致服务端超时(阈值8s)!")
                    ...
                    if send_cost > 0.5:
                        logger.warning(f"send_bytes耗时过长: {send_cost:.3f}s, 网络可能拥堵")
                ...
                logger.error(f"sender异常: {type(e).__name__}: {e}", exc_info=True)
                raise
